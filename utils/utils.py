...
这个文件目前只包含一个很关键的小工具：从 LLM 回复中提取 Python 代码块。它虽然简单，
但在论文映射里属于 coding operator 的协议支撑层：

- 论文把搜索对象定义为“代码空间中的候选解”；
- 而 LLM 输出往往是“解释 + 代码 + 额外文本”的混合体；
- 因此必须有一个稳定的工具把自然语言回复还原成可执行脚本。

复试时可以把它理解为：这是连接“大模型文本输出”和“解释器真正执行的 solution.py”之间的桥。
"""
...
def extract_python_code(text):
    ...
    这个函数之所以重要，是因为 AIDE 的 evaluator 只能处理真正的代码字符串，不能直接处理
    “夹杂解释文字的聊天回复”。如果这一步不稳，整棵 solution tree 就会被空脚本或半截代码污染。
    """
